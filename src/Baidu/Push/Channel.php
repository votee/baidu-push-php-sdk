...
namespace Baidu\Push;

use Baidu\Bae\BaeBase;
use Baidu\Bae\RequestCore;
use Baidu\Bae\ResponseCore;
use Baidu\Push\Exception\ChannelException;
...
class Channel extends BaeBase
{
    ...
    */
    public function __construct($apiKey = null, $secretKey = null, $arr_curlOpts = array())
    {
        ...
        if ($this->_checkString($secretKey, 1, 64)) {
            ...
            throw new ChannelException(
                "invalid param - secretKey[$secretKey],"
                . "which must be a 1 - 64 length string",
                self::CHANNEL_SDK_INIT_FAIL
            );
        }
        ...
        if (!is_array($arr_curlOpts)) {
            throw new ChannelException(
                'invalid param - arr_curlopt is not an array ['
                . print_r($arr_curlOpts, true) . ']',
                self::CHANNEL_SDK_INIT_FAIL
            );
        }
    ...
    protected function _getKey(&$opt, $opt_key, $member, $g_key, $env_key, $min, $max, $throw = true)
    {
        ...
        if ($throw) {
            throw new ChannelException(
                'no param (' . $dis[$opt_key] . ') was found',
        ...
        foreach ($params as $key => $value) {
            switch($key)
            {
                ...
                case self::CHANNEL_ID:
                    if (!is_numeric($value)) {
                        throw new ChannelException(
                            "CHANNEL_ID($value) is not numeric",
                            self::CHANNEL_SDK_PARAM
                        );
                    }
                    break;
                ...
                case self::TAG_NAME:
                    if (!is_string($value) || strlen($value) > 128) {
                        throw new ChannelException(
                            "TAG_NAME($value) must be a string and strlen <= 128",
                            self::CHANNEL_SDK_PARAM
                        );
                    }
                    break;
                case self::MESSAGE_TYPE:
                    if (!is_numeric($value) || $value < 0 || $value > 1) {
                        throw new ChannelException(
                            "invalid MESSAGE_TYPE($value) must be 0 or 1",
                            self::CHANNEL_SDK_PARAM
                        );
                    }
                    break;
                case self::NAME:
                    if (!is_string($value) || strlen($value) > 128) {
                        throw new ChannelException(
                            "IOS_CERT_NAME($value) must be a string and strlen <= 128",
                            self::CHANNEL_SDK_PARAM
                        );
                    }
                    break;
                case self::DESCRIPTION:
                    if (!is_string($value) || strlen($value) > 256) {
                        throw new ChannelException(
                            "IOS_CERT_DESCRIPTION($value) must be a string and strlen <= 256",
                            self::CHANNEL_SDK_PARAM
                        );
                    }
                    break;
    ...
    */
    ...
    protected function _commonProcess($paramOpt = null)
    {
        ...
        if (empty($ret)) {
            throw new ChannelException(
                'base control returned empty object',
    ...
    protected function _mergeArgs($arrNeed, $tmpArgs)
    {
        ...
        if (count($tmpArgs) - 1 != count($arrNeed) && count($tmpArgs) != count($arrNeed)) {
            ...
            $keys .= ')';
            throw new \Exception(
                'invalid sdk params, params' . $keys . 'are needed',
                self::CHANNEL_SDK_PARAM
            );
        }
        ...
        if (count($tmpArgs) - 1 == count($arrNeed) && !is_array($tmpArgs[count($tmpArgs) - 1])) {
            throw new \Exception(
                'invalid sdk params, optional param must be an array',
                self::CHANNEL_SDK_PARAM
            );
        }
